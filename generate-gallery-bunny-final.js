...
// Version ES Modules - CORRIGÉE pour gérer les espaces dans les noms de dossiers
...
function bunnyApiRequest(path) {
  return new Promise((resolve, reject) => {
    // Encoder correctement le path (gérer les espaces et caractères spéciaux)
    const encodedPath = path.split('/').map(p => encodeURIComponent(p)).join('/');
    
    const options = {
      hostname: 'storage.bunnycdn.com',
      path: `/${CONFIG.storageZoneName}/${encodedPath}`,
      method: 'GET',
      headers: {
    ...
    for (const item of items) {
      if (item.IsDirectory) {
        ...
        if (extensions.some(ext => fileName.endsWith(ext))) {
          ...
          // Encoder l'URL pour le CDN (remplacer espaces par %20)
          const encodedFilePath = filePath.split('/').map(p => encodeURIComponent(p)).join('/');
          
          allFiles.push({
            filename: item.ObjectName,
            url: `${CONFIG.cdnUrl}/${encodedFilePath}`,
            size: item.Length,
            path: filePath,
