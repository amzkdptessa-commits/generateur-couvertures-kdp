...
def gen_gallery_page(cat_data: Dict, sub_data: Dict, subsub_data: Dict = None, files: List = None) -> str:
  ...
  <script>
    ...
    function openLB(i){{lbIdx=i;const t=filtered[i];document.getElementById('lbImg').src=t.url;document.getElementById('lbTitle').textContent=t.name;document.getElementById('lbCounter').textContent=`${{i+1}}/${{filtered.length}}`;document.getElementById('lightbox').classList.add('active');document.body.style.overflow='hidden'}}
    ...
    function navLB(d){{lbIdx=(lbIdx+d+filtered.length)%filtered.length;const t=filtered[lbIdx];document.getElementById('lbImg').src=t.url;document.getElementById('lbTitle').textContent=t.name;document.getElementById('lbCounter').textContent=`${{lbIdx+1}}/${{filtered.length}}`}}
